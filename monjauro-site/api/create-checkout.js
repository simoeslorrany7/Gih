const PRODUCTS = {
  1: { name: "Monjaro Natural - 1 pote (120 capsulas)", amount: 5490 },
  2: { name: "Monjaro Natural - 2 potes (240 capsulas)", amount: 8490 },
  3: { name: "Monjaro Natural - 3 potes (360 capsulas)", amount: 10990 }
};
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(![11,14].includes(cpf.length)) return res.status(400).json({error:"CPF/CNPJ inválido."});
    ...
    if(phone.length>=10){
      ...
      if(number.length>=8 && number.length<=9){
        phoneObj={country:"+55",area,number};
      }
    }

    const origin = process.env.SITE_URL || `https://${req.headers.host}`;
    const reference = `GIGI-${Date.now()}`;
    ...
    if(!pay?.href) return res.status(502).json({error:"Checkout criado, mas sem link PAY.",checkout_id:data.id});
